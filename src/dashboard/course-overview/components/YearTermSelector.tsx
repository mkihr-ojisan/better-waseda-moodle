...
export const YearTermSelector: FC<YearTermSelectorProps> = (props) => {
    const items = useYearTermList(props.courses, props.timetableData);
 ...
 * @returns 年度・学期の選択肢
 */
function useYearTermList(
    courses: readonly Course[],
    timetableData: Partial<Record<string, TimetableData>>
): { value: YearTerm; label: string }[] {
    const [hideTabsBeforeEnrollment] = useConfig(ConfigKey.HideTabsBeforeEnrollment);
    const { value: enrollmentYear } = usePromise(
        () => (hideTabsBeforeEnrollment ? call("getEnrollmentYear") : Promise.resolve(undefined)),
        [hideTabsBeforeEnrollment]
    );
    ...
    return useMemo(() => {
        ...
        for (const course of courses) {
            ...
            if (timetable && timetable.length > 0) {
                ...
                yearTerms = YearTerm.fromInterval(course.date, "semester")
                    .filter(({ year }) => year <= currentYear) // 未来の分は表示しない
                    .filter(({ year }) => !hideTabsBeforeEnrollment || !enrollmentYear || enrollmentYear <= year); // 入学年度より前の分は表示しない
            } else {
                continue;
            }
        ...
        const sortedYearTerms = yearTerms.sort(YearTerm.compare);

        const yearTermsWithLabel = sortedYearTerms.map((yt) => ({
            value: yt,
            label: yt.toLocaleString(),
        }));

        return yearTermsWithLabel;
    }, [courses, enrollmentYear, hideTabsBeforeEnrollment, timetableData]);
}
