 ...
 */
export const login = combinePromise(async (loginInfo: Lazy<{ userId: string; password: string }>) => {
    try {
        const response = await fetchWithCredentials("https://wsdmoodle.waseda.jp/");
        let page: string | null = null;
        if (response.url === "https://wsdmoodle.waseda.jp/login/index.php") {
            ...
            if (
                ...
                page = await (
                    await postForm("https://wsdmoodle.waseda.jp/auth/saml2/sp/saml2-acs.php/wsdmoodle.waseda.jp", form)
                ).text();
            }

            if (!page) throw new InvalidResponseError("unknown error");
        } else {
            page = await response.text();
        }

        const sessionKey = page.match(/"https:\/\/wsdmoodle\.waseda\.jp\/login\/logout\.php\?sesskey=([^"]+)"/)?.[1];
