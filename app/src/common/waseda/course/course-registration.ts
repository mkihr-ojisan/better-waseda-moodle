...
import {
    CourseRegistrationPageUnderMaintenance,
    FetchCourseRegistrationInfoError,
...
export async function fetchCourseRegistrationInfo(): Promise<CourseRegistrationInfo[]> {
    try {
        ...
        if (page3.body.innerHTML.indexOf('ただいまメンテナンス中です。') >= 0) {
            throw new CourseRegistrationPageUnderMaintenance();
        }
        ...
        for (const tr of Array.from(page3.getElementsByTagName('table')[12]?.getElementsByTagName('tr') ?? []).slice(
            1
        )) {
        ...
        throw new FetchCourseRegistrationInfoError(error);
    }
}
