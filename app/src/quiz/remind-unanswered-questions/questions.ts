...
function checkIfAnswered(elem: Element, type: QuestionType): boolean {
    switch (type) {
        ...
        case 'ddmarker':
            return Array.from(elem.querySelectorAll('.ddform > input')).some(e => (e as HTMLInputElement).value !== '');
        case 'ddimageortext':
        case 'ddwtos':
            return Array.from(elem.querySelectorAll('input[type=hidden].placeinput')).every(e => (e as HTMLInputElement).value !== '0');
        case 'description':
            return true;
        case 'essay':
            {
                ...
                if (htmlEditorContent) {
                    if (htmlEditorContent.innerHTML.trim() !== '')
                        return true;
                    if (htmlEditorContent.querySelector('img') || htmlEditorContent.querySelector('audio') || htmlEditorContent.querySelector('video'))
                        return true;
                } else {
                ...
                if (filePicker) {
                    if (!filePicker.classList.contains('fm-noitems'))
                        return true;
                }
            }
            return false;
